...
public class Tank extends GameObject implements Poolable {
    ...
    public void updateWeapon(float dt) {
        if (weapon.getType() == Weapon.Type.GROUND && target != null) {
           ...
           if (destination.dst(position) < 378) {
    ...
    public void render(SpriteBatch batch) {
        float linehp = ((float) hp) / hpMax;
        if (gc.isTankSelected(this) && ownerType != Tank.Owner.AI) {
            float c = 0.7f + (float) Math.sin(lifeTime * 8.0f) * 0.3f;
            batch.setColor(c, c * linehp, c * linehp, 1.0f);
        } else {
            batch.setColor(1, linehp, linehp, 1.0f);
